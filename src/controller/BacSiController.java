package controller;

import java.io.IOException;
import java.util.ArrayList;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import bean.BacSiBEAN;
import bean.BenhAnBEAN;
import bo.BacSiBO;
import bo.BenhAnBO;

/**
 * Servlet implementation class BacSiController
 */
@WebServlet("/BacSiController")
public class BacSiController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public BacSiController() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		
		
		ArrayList<BacSiBEAN> ds = new ArrayList<>();
		BacSiBO bacSiBO = new BacSiBO();
		try {
			ds=bacSiBO.getBacSi();
			request.setAttribute("ds", ds);
			String DoctorID = request.getParameter("ma");
			if(DoctorID!=null) {
				BenhAnBO benhAnBO = new BenhAnBO();
				ArrayList<BenhAnBEAN> dsBenhAn = new ArrayList<>();
				dsBenhAn = benhAnBO.timBenhAn(DoctorID);
				request.setAttribute("dsBenhAn", dsBenhAn);
			}
			
			
			RequestDispatcher rd = request.getRequestDispatcher("BenhAn.jsp");
			rd.forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
	}

}
